...
#==============================================================================#
# Ver que onda con los datos filtrados
#==============================================================================#
#importar tabla
tabla_na_filtrada<-importar_datos(nombre_archivo = "inflacion_mas_banco.csv",carpeta = "processed")

#analisis de na
analisis_na_pais<-analizar_na(tabla = tabla_na_filtrada,grupo = "country")
analisis<-analisis_na_pais$por_columna%>%arrange(na_om01)
print(analisis,n=Inf)

# Quinqueneos
df_quinquenal <- tabla_na_filtrada %>%
  # 1. Crear la variable 'quinquenio'
  mutate(
    # Calcula el inicio del periodo de 5 años:
    # (year - 1970) %/% 5 da 0 para 1970-1974, 1 para 1975-1979, etc.
...
# ALGO
columnas_a_promediar<-c("inflacion", "di01","dm02", "om01")
df_promedios_final_filtrada <- df_quinquenal %>%
  # Agrupar por país Y por el periodo de 5 años
  group_by(country, quinquenio) %>%
  # Aplicar la función summarise(across()) para promediar las columnas deseadas
  summarise(
...
print(head(df_promedios_final_filtrada))

exportar_data(data = df_promedios_final_filtrada, nombre = "quinquenios_filtrada_p3", carpeta = "clean")
